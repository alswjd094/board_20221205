...
//    //회원(1) : 게시글(N) 연관관계(on delete cascade)
//    @OneToMany(mappedBy = "memberEntity", cascade=CascadeType.REMOVE, orphanRemoval = true, fetch = FetchType.LAZY)
//    private List<BoardEntity> boardEntityList = new ArrayList<>();
//
//    //회원(1) : 댓글(N) 연관관계(on delete cascade)
//    @OneToMany(mappedBy = "memberEntity", cascade=CascadeType.REMOVE, orphanRemoval = true, fetch = FetchType.LAZY)
//    private List<CommentEntity> commentEntityList = new ArrayList<>();
    ...
    private void preRemove(){
        ...
        commentEntityList.forEach(comment ->{
            comment.setMemberEntity(null); //comment_table member_id
            comment.setCommentWriter("탈퇴회원"); //comment_table comment_writer
        });
    }
